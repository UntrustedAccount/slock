...
static void drawlogo(Display *dpy, struct lock *lock, int color) {
  if (lock->bgmap)
    ...
    XSetWindowBackground(dpy, lock->win, lock->colors[BACKGROUND]);
...
int main(int argc, char **argv) {
  ...
  monitors = XRRGetMonitors(dpy, RootWindow(dpy, XScreenNumberOfScreen(scr)),
                            True, &number_of_monitors);

  int i;
  for (i = 0; i < number_of_monitors; i++) {
    imlib_blend_image_onto_image(
        buffer, 0, 0, 0, background_image_width, background_image_height,
